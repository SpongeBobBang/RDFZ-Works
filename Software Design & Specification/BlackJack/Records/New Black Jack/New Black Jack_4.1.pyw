...
def LoadInterface():
    ...
    cv=Canvas(root,width=500,height=600)
    cv.pack()
    LoadBackGround()
...
def DisplayPlayers():
    ...
    for i in range (1,PlayerNumber+1):
        labelPlayerName=Label(root,text=PlayerDisplay[i]+":",anchor="w",bg="#bbbbbb",font=("造字工房悦黑（非商用）纤细体",30,"bold")
...
def ShowCard(PlayerIndex):
    ...
    for i in range(1,3):
        labelThisCard=Label(root,text=Card[PlayerIndex][i-1],font=("造字工房悦黑（非商用）纤细体",30,"bold"))
    ...
    for i in range(3,len(Card[PlayerIndex])+1):
        ...
        if PlayerIndex == 1:
            temp = Card[1][i-1]
        labelThisCard=Label(root,text=temp,font=("造字工房悦黑（非商用）纤细体",30,"bold"))
        labelThisCard.place(height=50,width=50,x=XCoor,y=YCoor)
        XCoor = XCoor + 60

def HitButton(CurrentPlayer):
    global ButtonHit
    labelPlayerName=Label(root,text=PlayerDisplay[CurrentPlayer]+"'s turn:",anchor="w")
...
def HitDone(event):
    ...
    if InfoShown == True:
        ...
        labelShowInfo.place(height=50,width=400,x=100,y=10+60*PlayerNumber+200)
...
def ShowInfo(event):
    ...
    if InfoShown == False:
        ...
        labelShowInfo=Label(root,text=temp,anchor="w")
        labelShowInfo.place(height=50,width=400,x=100,y=10+60*PlayerNumber+200)
        InfoShown = True
    elif InfoShown == True:
        InfoShown = False
...
def Next(event):
    ...
    if x >= 2 and x < PlayerNumber:
        ...
        BankerShown = True
        ShowCard(1)
    labelPlayerName=Label(root,text=PlayerDisplay[x]+"'s turn:",anchor="w")
    labelPlayerName.place(height=50,width=120,x=5,y=10+60*PlayerNumber+50)
...
def RestartButton():
    ButtonRestart=Button(root,text="restart")
    ButtonRestart.place(height=40,width=60,x=400,y=10)
