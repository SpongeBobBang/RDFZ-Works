...
def ShowCard(PlayerIndex):
    ...
    for i in range(3,len(Card[PlayerIndex])+1):
        ...
        if PlayerIndex == 1:
            temp = Card[1][i-1]
        labelThisCard=Label(root,text=temp,bg="yellow")
        labelThisCard.place(height=50,width=50,x=XCoor,y=YCoor)
        XCoor = XCoor + 60
...
def ShowInfo(event):
    ...
    if InfoShown == False:
        ...
        labelShowInfo.place(height=50,width=400,x=100,y=10+60*PlayerNumber+200)
        InfoShown = True
        labelTest=Label(root,text=InfoShown,anchor="w")
        labelTest.place(height=50,width=50,x=400,y=400)
    elif InfoShown == True:
        InfoShown = False
        labelShowInfo.place_forget()
        labelTest=Label(root,text=InfoShown,anchor="w")
...
def Next(event):
    ...
    if x >= 2 and x < PlayerNumber:
        ...
        BankerShown = True
        ShowCard(1)
    labelPlayerName=Label(root,text=PlayerDisplay[x]+"'s turn:",anchor="w",bg="yellow")
    labelPlayerName.place(height=50,width=120,x=5,y=10+60*PlayerNumber+50)
