...
def NumberInput(event):
    ...
    labeltest=Label(root,fg="white",text=PlayerNumber,bg="#030303",font=("造字工房悦黑（非商用）纤细体",15,"bold"))
    labeltest.place(height=80,width=80,x=400,y=400)
...
def NoticeButton():
    ButtonNotice=Button(root,fg="#bdd7ee",bg="#050505",activebackground="#bbbbbb",text="NOTICE",font=("造字工房悦黑（非商用）纤细体",18,"bold"))
    ButtonNotice.place(height=60,width=120,x=335,y=10+70*PlayerNumber+90)
    ButtonNotice.bind("<Button-1>",Notice)

def Notice(event):
    notice=Tk()
    notice.title("Notice")
    notice.resizable(0, 0)
    notice.bind("<Destroy>",root.focus_set())
    cv=Canvas(notice,width=520,height=350)
    cv.pack()
    temp = ["Welcome to BlackJack!",
            "This game allows 2 to 6 players. ",
            "Each player gets drawn 2 cards from the pack and are allowed to draw cards in their turn. ",
            "The one reaches the biggest number under or equal to 21 wins. ",
            "Each player compares with the banker. ",
            "",
            "You can use quick keys in this game. ",
            "Numbers for total number of plaers; ",
            "'H' for hit, 'S' for stand; ",
            "'I' to show your cards, 'P' or 'E' to pass on to the next player; ",
            "'R' to restart game."]
    YCoor = 5
    labelBackGroundNotice=Label(notice,bg="#030303")
    labelBackGroundNotice.place(height=4000,width=4000,x=0,y=0)
    labelNotice=Label(notice,bg="#030303",fg="#bdd7ee",text=temp[0],font=("造字工房悦黑（非商用）纤细体",15,"bold"))
    labelNotice.place(height=35,width=520,x=10,y=YCoor)
    YCoor = 45
    for i in range (1,len(temp)):
        labelNotice=Label(notice,bg="#030303",fg="#bdd7ee",text=temp[i],anchor=W,font=("造字工房悦黑（非商用）纤细体",10,"bold"))
        labelNotice.place(height=25,width=520,x=5,y=YCoor)
        YCoor = YCoor + 30
