...
class Coefficient():
	def __init__(self,trigType,coef):
		...
		self.termNoA = None #Term number to output/Number of terms at numerator for tan 
		self.termNoB = None #Term number to output/Number of terms at denominator for tan 
		self.coefListTot = [] #Coefficient list, of Re&Im  
		self.coefListCos = [] #Coef list, of Re parts 
		self.coefListSin = [] #Coef list, of Im parts 
		self.coefListA = [] #Coef list with joint trigonometry: sin&cos, initially/Coef list at numerator for tan
		self.coefListB = [] #Coef list with simplified, sole trig, finally/Coef list at denominator for tan

	def GetCoefListA(self):
		dCoefSignA = {1:1,2:1,3:-1,0:-1}
		for i in range(self.termNoTotal):
			self.coefListTot.append(dCoefSignA[(i+1)%4]*Combination(self.coef,i))
		dGoTrig = {"cos":self.GetCoefListCosA,"sin":self.GetCoefListSinA,"tan":self.GetCoefListTanA}
		dGoTrig[self.trigType]()

	def GetCoefListCosA(self):
		for i in range(0,self.termNoTotal,2):
			self.coefListCos.append(self.coefListTot[i])
		self.coefListA = self.coefListCos

	def GetCoefListSinA(self):
		for i in range(1,self.termNoTotal,2):
			self.coefListSin.append(self.coefListTot[i])
		self.coefListA = self.coefListSin

	def GetCoefListTanA(self):
		self.GetCoefListCos()

	def GetCoefListB(self):
		dGoTrig = {"cos":self.GetCoefListCSB,"sin":self.GetCoefListCSB,"tan":self.GetCoefListTanB}
		dGoTrig[self.trigType]()

	def GetCoefListCSB(self): #For cos&sin, to eliminate the other trig
		self.termNoA = len(self.coefListA)
		dCoefSignB = {0:1,1:-1}
		for i in range(self.termNoA):
			temp = 0
			t = 0 
			for j in range(i,self.termNoA):
				temp += dCoefSignB[t%2]*Combination(j,i)*self.coefListA[j]
				t += 1
			self.coefListB.append(temp)

	def GetCoefListTanB(self):
		self.GetCoefListSin()
