...
class Coefficient():
	def __init__(self,trigType,coef):
		self.trigType = trigType
		self.coef = coef
		self.termNoTotal = self.coef + 1 #Total number of terms, real and imaginary 
		self.coefListTot = [] #List containing all coefficients, combined 
		self.coefListCos = [] #List containing coefficients of real parts 
		self.coefListSin = [] #List containing coefficients of imaginary parts 
	...
	def GetCoefListSub(self):
		...
		dGoTrig = {"cos":self.GetCoefListCos,"sin":self.GetCoefListSin}
