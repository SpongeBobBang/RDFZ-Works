...
class Expoential():
	...
	def GetConstant(self):
		...
		if self.termNo != self.variedTermNo: #There's a constant value term 
			self.constValue = int(self.GetTermSign(self.trigType,self.expo,self.expo/2)*Combination(self.expo,self.expo/2))
			start += 1
		for i in range(self.variedTermNo):
			self.constList.append(int(self.GetTermSign(self.trigType,self.expo,start+i)*2*Combination(self.expo,start+i)))
...
#--------------------------------
def Main():
	print("Welcome to Complex Number Cracker.\nType in the kind of quetions you want to slove.")
